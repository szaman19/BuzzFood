...
def getTags(imageUrlDict):
	...
	for imageURL in imageUrlValues:
		...
		request = requests.get("https://api.clarifai.com/v1/tag/",headers = header,params=data)
		JSONTags = request.json()
...
def gifSearch(tagDict):
	...
	tagKeys = tagDict.keys()
	for tag in tagKeys:
		if tagDict[tag] > len(tagDict) / float(10):
	...
	params = {"api_key" : "dc6zaTOxFJmzC", 'q' : searchString, "limit" : 3}
	url = "http://api.giphy.com/v1/gifs/search"
	request = requests.get(url,params = params)
	gifData = request.json()['data']

	gifLinks = []
	for data in gifData:
		gifLinks.append(data['url'].encode('utf-8'))
	return gifLinks
